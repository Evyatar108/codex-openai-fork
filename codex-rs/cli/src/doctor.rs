    ...
    if doctor_managed_by_npm(current_exe.as_deref()) {
        match npm_global_root_check() {
            ...
            NpmRootCheck::Mismatch {
                ...
                // SANDBOX PATCH: fork package is @gim-home/codex (not the upstream @openai
                // scope package).
                summary =
                    "npm install -g @gim-home/codex would update a different install".to_string();
...
fn compare_npm_package_roots(running_package_root: &Path, npm_root: &Path) -> NpmRootCheck {
    // SANDBOX PATCH: the fork installs as @gim-home/codex, not the upstream @openai scope
    // package. Without this, the npm-root comparison always Mismatches and `codex doctor`
    // falsely Fails on a correct fork install.
    let npm_package_root = npm_root.join("@gim-home").join("codex");
...
mod tests {
    ...
    fn compare_npm_package_roots_detects_match() {
        let running = PathBuf::from("/prefix/lib/node_modules/@gim-home/codex");
        let npm_root = PathBuf::from("/prefix/lib/node_modules");
        assert_eq!(
            compare_npm_package_roots(&running, &npm_root),
            NpmRootCheck::Match {
                package_root: npm_root.join("@gim-home").join("codex")
            }
        );
    }
    ...
    fn compare_npm_package_roots_detects_mismatch() {
        let running = PathBuf::from("/old/lib/node_modules/@gim-home/codex");
        let npm_root = PathBuf::from("/new/lib/node_modules");
        assert_eq!(
            compare_npm_package_roots(&running, &npm_root),
            NpmRootCheck::Mismatch {
                running_package_root: running,
                npm_package_root: npm_root.join("@gim-home").join("codex"),
            }
        );
    }
