...
async fn install_latest_standalone() -> Result<()> {
    // SANDBOX PATCH: neutralized. Upstream fetched the chatgpt.com installer script and
    // piped it into /bin/sh — an outbound network egress plus a remote-script-execution
    // vector. The fork has no install.sh self-updater, so this is a no-op. The daemon still
    // launches and restarts the managed binary at its fixed path; it just no longer
    // self-fetches a new one. Removing the body also drops the reqwest::get + /bin/sh pipe.
    Ok(())
}
