...
pub async fn run_main_with_transport_options(
    ...
    // unconditionally (lean-on-upstream replacement for the retired Layer-2 set_enabled
    // override). DisabledByRequirements makes ensure_remote_control_allowed() reject every
    // enable RPC -- including message_processor::handle_config_mutation reacting to a user
    // `features.remote_control = true` ConfigWrite -- so the websocket can never enroll at
    // chatgpt.com. See CLAUDE.md / AGENTS.override.md "Remote control is force-disabled at THREE layers".
