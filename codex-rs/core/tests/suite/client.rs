...
// SANDBOX PATCH: the WIP copilot-client integration tests (test_copilot_auth,
// copilot_client_fixture -> CopilotSessionFixture, configure_copilot_session_for_tests,
// and the copilot_* tests) were removed during the v0.140.0 rebase. They referenced
// fork test infrastructure that was never committed (the responses-URL test side-channel
// that redirects /responses to a wiremock server does not exist). The copilot transport
// stays covered by model-provider/src/copilot.rs unit tests; re-adding end-to-end
// copilot-client tests is tracked as a follow-up (codex-rebase-debt-fix-client-copilot-fixture).
