...
fn candidate_filenames(config: &Config) -> Vec<&str> {
    ...
    let auto_load_claude_md = config.features.enabled(Feature::AutoLoadClaudeMd);
    let mut names: Vec<&str> = Vec::with_capacity(
        2 + config.project_doc_fallback_filenames.len() + usize::from(auto_load_claude_md),
    );
    ...
    if auto_load_claude_md && !names.contains(&CLAUDE_MD_FILENAME) {
        names.push(CLAUDE_MD_FILENAME);
    }
    names
}
