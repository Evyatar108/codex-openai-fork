...
pub fn build_models_manager(
    ...
    // SANDBOX PATCH: `create_model_provider` routes Copilot sessions through
    // `CopilotModelProvider`, which produces a `models_manager` that uses the
    // configured Copilot provider rather than the hardcoded OpenAI provider.
