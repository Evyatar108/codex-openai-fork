...
pub fn build_models_manager(
    ...
    // SANDBOX PATCH: pass the real Copilot provider into ModelsManager instead of always
    // using the hardcoded OpenAI provider, so Copilot sessions get the synthetic catalog.
    let models_provider = if config.model_provider.is_copilot() {
