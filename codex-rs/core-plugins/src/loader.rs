...
async fn load_mcp_servers_from_file(
    ...
    // SANDBOX PATCH: invariant 20 — substitute ${CLAUDE_PLUGIN_ROOT}/${CODEX_PLUGIN_ROOT}
    // before upstream's structured parse so plugins shipping a single Claude-style
    // .mcp.json work in codex unmodified. See docs/implementation/patch-surface.md §16.
    let contents =
        crate::mcp_substitution::apply_plugin_root_substitution_to_contents(plugin_root, &contents);
    let parsed =
