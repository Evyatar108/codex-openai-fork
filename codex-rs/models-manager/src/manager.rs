...
impl ModelsManager {
    ...
    pub fn new_with_provider(
        ...
        let is_copilot = provider_info.is_copilot();
        ...
        let remote_models = if is_copilot && !has_explicit_catalog {
    ...
    pub async fn get_default_model(
        ...
        if self.provider.info().is_copilot() && matches!(self.catalog_mode, CatalogMode::Default) {
    ...
    async fn refresh_available_models(&self, refresh_strategy: RefreshStrategy) -> CoreResult<()> {
        ...
        if self.provider.info().is_copilot() {
            return Ok(());
        }
