...
impl HistoryCell for UpdateAvailableHistoryCell {
    fn display_lines(&self, width: u16) -> Vec<Line<'static>> {
        ...
        // SANDBOX PATCH: redirect upstream openai/codex links to the fork releases page.
        // command_str() now returns the fork releases URL, so phrase it as "See <url>".
        let update_instruction = if let Some(update_action) = self.update_action {
            line![
                "See ",
                update_action.command_str().cyan().underlined(),
                " to update."
            ]
        } else {
            line![
    ...
    fn raw_lines(&self) -> Vec<Line<'static>> {
        let update_instruction = if let Some(update_action) = self.update_action {
            format!("See {} to update.", update_action.command_str())
        } else {
            "See https://github.com/gim-home/codex/releases for installation options.".to_string()
        };
        vec![
